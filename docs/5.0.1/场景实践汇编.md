...
本文按常见业务场景串联 API，细节仍以各模块专文为准（见 [README.md](index.md)）。

## 1. 应用启动：加载默认数据源并查询一行

1. 确保 Keel 根配置已加载，且存在 **`mysql.<name>.*`**（见 [配置与即时查询.md](./配置与即时查询.md)）。
...
失败时库内会尝试 **rollback** 并抛出 **`KeelMySQLException`**（语义见 [数据源与连接池.md](./数据源与连接池.md)）。**不要在
...
返回 **`PaginationResult`**，可 **`toJsonObject()`** 给 HTTP 层（见 [结果集分页与映射.md](./结果集分页与映射.md)）。
...
占位符为 **`{name}`** 文本替换，用户参与部分须先 **`Quoter`** 或校验（见 [原始SQL与模板SQL.md](./原始SQL与模板SQL.md)）。
...
之后通过 **`RunnableStatement.execute()`** 的语句会写审计事件（见 [异常与SQL审计.md](./异常与SQL审计.md)）。

## 8. 一次性运维脚本（无长期池）

**`config.instantQuery(vertx, "SELECT ...")`** 或 **`config.instantQueryForStream(keel, sql, batch, handler)`
**（见 [配置与即时查询.md](./配置与即时查询.md)）。注意 **`instantQuery`** 为 Technical Preview，且不适合高并发短连接场景。
